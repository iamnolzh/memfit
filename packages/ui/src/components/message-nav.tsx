...
const hasChanges = (diffs: any) => {
  if (!diffs) return false
  const list = Array.isArray(diffs) ? diffs : [diffs]
  return list.some(x => (x.additions || 0) + (x.deletions || 0) > 0)
}
              ...
              <Switch>
                ...
                <Match when={local.size === "normal"}>
                  <button data-slot="message-nav-message-button" onClick={handleClick}>
                    <Show
                      when={hasChanges(message.summary?.diffs)}
                      fallback={<Icon name="speech-bubble" size="small" class="text-text-weak shrink-0" />}
                    >
