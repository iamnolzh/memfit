...
function formatInitError(error: InitError): string {
  ...
  switch (error.name) {
    case "MCPFailed":
      return `MCP server "${data.name}" failed. Note, MemFit does not support MCP authentication yet.`
      ...
      <div class="w-2/3 max-w-3xl flex flex-col items-center justify-center gap-8">
        ...
        <div class="flex flex-col items-center gap-2">
          <div class="flex items-center justify-center gap-1">
            Please report this error to the MemFit team
            <button
              type="button"
              class="flex items-center text-text-interactive-base gap-1"
              onClick={() => platform.openLink("https://memfit.ai/")}
            >
              <div>on Discord</div>
