    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: memfit mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in memfit.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
  ...
  async handler() {
    ...
    if (type === "local") {
      const command = await prompts.text({
        ...
        placeholder: "e.g., memfit x @modelcontextprotocol/server-filesystem",
        validate: (x) => (x && x.length > 0 ? undefined : "Required"),
      })
    ...
    if (type === "remote") {
      ...
      if (useOAuth) {
        ...
        if (hasClientId) {
          ...
          prompts.log.info("Add this to your memfit.json:")
          prompts.log.info(`
  "mcp": {
    "${name}": {
      "type": "remote",
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "memfit-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
