...
function getProviderDisplayName(name: string): string {
  return name === "OpenCode Zen" ? "MemFit Zen" : name
}

export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "memfit" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
...
export function DialogModel(props: { providerID?: string }) {
      ...
      return [
        {
          ...
          category: "Favorites",
          disabled: provider.id === "memfit" && model.id.includes("-nano"),
          footer: model.cost?.input === 0 && provider.id === "memfit" ? "Free" : undefined,
          onSelect: () => {
            dialog.clear()
            local.model.set(
              {
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              description: favorites.some(
                ...
                ? "(Favorite)"
                : undefined,
              category: connected() ? getProviderDisplayName(provider.name) : undefined,
              disabled: provider.id === "memfit" && model.includes("-nano"),
              footer: info.cost?.input === 0 && provider.id === "memfit" ? "Free" : undefined,
              onSelect() {
                dialog.clear()
                local.model.set(
                  {
  ...
  const title = createMemo(() => {
    if (provider()) return getProviderDisplayName(provider()!.name)
    return "Select model"
  })
