...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const agent = iife(() => {
      ...
      return {
        list() {
          return agents().map((x) => ({ ...x, displayName: getAgentDisplayName(x.name) }))
        },
        current() {
          const agent = agents().find((x) => x.name === agentStore.current)!
          return { ...agent, displayName: getAgentDisplayName(agent.name) }
        },
        getDisplayName(name: string): string {
          return getAgentDisplayName(name)
        },
        set(name: string) {
    ...
    const model = iife(() => {
      ...
      return {
        ...
        parsed: createMemo(() => {
          ...
          return {
            provider: getProviderDisplayName(provider?.name ?? value.providerID),
