...
test("updates config and writes to file", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const writtenConfig = JSON.parse(await Bun.file(path.join(Global.Path.config, "config.json")).text())
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "opencode.json"),
        JSON.stringify({ $schema: "https://memfit.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
