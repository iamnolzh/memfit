...
export async function runUpdater({ alertOnFail }: { alertOnFail: boolean }) {
  ...
  if (!update) {
    if (alertOnFail)
      await message("You are already using the latest version of MemFit", { title: "No Update Available" })
    return
  }
  ...
  const shouldUpdate = await ask(
    `Version ${update.version} of MemFit has been downloaded, would you like to install it and relaunch?`,
